...
namespace Windows.Kinect
{
    ...
    public sealed partial class BodyFrame
    {
        ...
        {
            //UnityEngine.Debug.Log("GettingBodyData");

            if (_pNative == RootSystem.IntPtr.Zero)
            {
                UnityEngine.Debug.Log("_pNative == Zero");
                throw new RootSystem.ObjectDisposedException("BodyFrame");
            }
            //UnityEngine.Debug.Log("setting _bodies_idx");

            int _bodies_idx = 0;
            //UnityEngine.Debug.Log("setting _bodies");
            var _bodies = new RootSystem.IntPtr[bodies.Count];
            //UnityEngine.Debug.Log("Starting Body Loop");
            for (int i = 0; i < bodies.Count; i++)
            {
                //UnityEngine.Debug.Log("BFrame iterating");
                if (bodies[i] == null)
                {
                    //UnityEngine.Debug.Log("Adding new body");
                    bodies[i] = new Body();
                }
